...
- Utilizamos el desarrollo asociado a sesiones. Definiremos únicamente una variable de sesión (index a utilizar ```$_SESSION['user']```) en la que guardaremos la variable ```$user``` inicializada a partir del objeto ```user```, el cual debe de contener, además de los atributos ya asignados (```email``` y ```password```) , los atributos:
    - Id
    - Name
